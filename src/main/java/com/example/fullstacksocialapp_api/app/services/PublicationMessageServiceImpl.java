...
public class PublicationMessageServiceImpl implements PublicationMessageService {
    private static final String ENTITY = "Publication Message";
    ...
    @Override
    public PublicationMessage update(Long productId, PublicationMessage request) {
        ...
        return publicationMessageRepository.findById(productId).map(publicationMessage ->
    ...
    public ResponseEntity<?> delete(Long productId) {
        return publicationMessageRepository.findById(productId).map(product -> {
            publicationMessageRepository.delete(product);
